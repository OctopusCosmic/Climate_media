...
def process_highlighted_labeled_content(state_abbr,df_f1):
    ...
    df_label_a_state["content"] = df_label_a_state.apply(lambda row: get_content(row.location ,row.char_start ,row.char_end,df_f1), axis=1)
...
def main():
    ...
    df_label_a_state["label"] = df_label_a_state["label"].apply(lambda x: x.replace(" ",""))
    df_label_a_state=df_label_a_state.drop(columns=['row', "location"])
